...
int main()
{
    ...
    do
    {
        ...
        error = fabs(b-a);
        if(fabs(f(x))<epsilon || error<epsilon || iteration==15){
            break;
        }      
    } while (1);
    //printf("Since,aproximate error<error tolerance i.e %.4lf<%.3lf, the root is %.3lf",error,epsilon,x);
